...
fn build_circuit() -> Result<(ZkWasmLoader<Bn256>, ExecutionArg)> {
...
mod tests {
    ...
    rusty_fork_test! {
    #[test]
        fn test_rlp_cont_run(){
            let (loader, args) = build_circuit().unwrap();
            let result = loader.run(args).unwrap().result;
            assert!(result == Some(RuntimeValue::I32(1440)));
        }
    }
}